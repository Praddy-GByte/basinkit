...
even showed `auth=none, commercial=yes` — a reader would reasonably conclude
...
| **Nodata averaged into composites** | Optical products encode "no observation" as 0. A basin straddles several MGRS tiles, so most scenes cover a fraction of it — an unmasked Sentinel-2 median came back **87% zero**. |
...
| **Rasters exported without nodata** | Correctly masked in memory, then written to GeoTIFF with no declared nodata — QGIS paints the outside solid black and rasterio returns no mask. The clip silently stopped existing the moment the file left Python. |
